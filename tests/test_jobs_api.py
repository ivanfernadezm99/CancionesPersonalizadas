...
from app.jobs import count_active_jobs, create_job, get_job, update_status
...
@pytest.mark.asyncio
async def test_create_job_generates_uuid(
    db_path: str, sample_request: GenerateRequest,
) -> None:
...
@pytest.mark.asyncio
async def test_get_job_params_json(
    db_path: str, sample_request: GenerateRequest,
) -> None:
...
@pytest.mark.asyncio
async def test_create_job_sets_created_at(
    db_path: str, sample_request: GenerateRequest,
) -> None:
...
@pytest.mark.asyncio
async def test_create_job_estimated_remaining(
    db_path: str, sample_request: GenerateRequest,
) -> None:
...
@pytest.mark.asyncio
async def test_update_status_with_extra(
    db_path: str, sample_request: GenerateRequest,
) -> None:
...
@pytest.mark.asyncio
async def test_update_status_records_transition(
    db_path: str, sample_request: GenerateRequest,
) -> None:
    ...
    try:
        await init_db(conn)
        sql = (
            "SELECT from_status, to_status, error"
            " FROM job_transitions WHERE job_id = ? ORDER BY id"
        )
        cursor = await conn.execute(sql, (job_id,))
...
@pytest.mark.asyncio
async def test_count_active_jobs_includes_queued(
    db_path: str, sample_request: GenerateRequest,
) -> None:
...
@pytest.mark.asyncio
async def test_count_active_jobs_excludes_complete(
    db_path: str, sample_request: GenerateRequest,
) -> None:
...
@pytest.mark.asyncio
async def test_count_active_jobs_excludes_failed(
    db_path: str, sample_request: GenerateRequest,
) -> None:
...
@pytest.mark.asyncio
async def test_count_active_jobs_multiple(
    db_path: str, sample_request: GenerateRequest,
) -> None:
