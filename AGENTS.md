...
- **Landing de canciones** (`landing.component.ts`): portada informativa + `Conectado como <nombre>` + CTAs a rutas protegidas. Tiene un **formulario email/password liviano como fallback** para usuarios `Interno` que llegan directo a `/canciones/landing` sin pasar por POSFrontReform. El `authGuard` de las rutas protegidas también hace `forceLogin(returnUrl)` → POSFrontReform si no hay token. El flujo **principal** para Google es POSFrontReform (One Tap) → redirect con `?token=`.
