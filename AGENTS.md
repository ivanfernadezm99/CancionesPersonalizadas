...
### Autenticación — dueño único: POSFrontReform

⚠️ **Este módulo NO tiene login propio.** Toda la autenticación vive en **POSFrontReform** (`~/Descargas/POSFrontReform/`, Angular).

- **Flujo único**: POSFrontReform (Google One Tap + email/password `authType: Interno/Externo`) → `POST /api/Auth/Login` o `loginWithGoogle` en POSBackend → POSBackend acuña JWT HS256 → POSFrontReform redirige a `poscuentascorrientes-stage.../#/canciones/landing?token=<JWT>&returnUrl=...` → `captureTokenFromUrlOnce()` (en `main.ts`/`TokenSyncService`) guarda el token en `localStorage` → el módulo de canciones lo lee con `getToken()`/`decodeJwtPayload()` y lo envía vía `authInterceptor` a ambos backends.
- **Landing de canciones** (`landing.component.ts`): portada informativa + `Conectado como <nombre>` (valida `exp` y borra token vencido) + CTAs a rutas protegidas (`/canciones/create`, `/canciones/mis-canciones`). **Sin login propio**: `authGuard` hace `forceLogin(returnUrl)` → POSFrontReform (Google One Tap / Interno) → redirect con `?token=` → `captureTokenFromUrlOnce()` guarda en `localStorage`. Usuarios `Interno` también entran por POSFrontReform, no por form en la landing.
- **Claims del JWT**: POSBackend (`AuthApplication.cs:GenerateTokenAsync`) usa `ClaimTypes.NameIdentifier/Email/Name/Role`. `JwtSecurityTokenHandler` con `DefaultOutboundClaimTypeMap` puede emitirlos como `sub`/`email`/`unique_name`/`role` (cortos) o como URIs completas; el backend de canciones (`app/auth/middleware.py`, `app/auth/dependencies.py`) acepta **ambos** formatos. La landing extrae el display name probando `unique_name` → URIs → `name`/`given_name` → prefijo de `email`.
- **Expiración**: el JWT expira según `Jwt:Expires` de POSBackend. La landing valida `exp` client-side (`hasValidToken()`) y borra tokens vencidos para no mostrar `Conectado como` inconsistente con el 401 que daría `/api/projects/mine`.
...
- **POSBackend** (rama `staging`): el login con Google auto-crea **usuario mínimo** (`Email`, `UserName = Name ?? Email`, `AuthType=GOOGLE`, `State=1` — sin sucursal/rol/negocio) y además un `Client` mínimo (mismo email, sin sucursal) para que exista registro de cliente. El teléfono se guarda en `Client.Phone`. Endpoint nuevo `POST /api/Auth/UpdatePhone` (valida y persiste, devuelve JWT fresco con el claim `Phone`). El claim `Phone` se agrega al JWT cuando el Client tiene teléfono. `GET /api/UserManagement/current-user` ya devuelve `PhoneNumber` real.
- **POS alta es separada**: el alta del POS (Interno, con sucursal/roles/negocio) **no se toca** — carga datos completos de negocio y es otro flujo. Canciones/Google nunca crea sucursal ni rol; solo el mínimo para identidad + teléfono.
