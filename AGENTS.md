...
- **NO duplicar login en la landing de canciones**: `src/app/canciones-personalizadas/landing/landing.component.ts` es solo una portada informativa + `Conectado como <nombre>` + CTAs a rutas protegidas (`/canciones/create`, `/canciones/mis-canciones`). El `authGuard` de esas rutas hace `forceLogin(returnUrl)` si no hay token válido. Cualquier formulario email/password en la landing es redundante y diverge del flujo Google/phone de POSFrontReform.
